...
class Solution {
    ...
    /*
     * 解决方案：动态规划
     *        递推方程：dp[i] = cost[i] + min(dp[i-1], dp[i-2])
     *        只有两种爬法，只关心前两步的情况，再之前的状态没用了；
     *           使用minCostFirst、minCostSecond来表示
     * 
     * 
     * 时间复杂度：O(n)
     * 空间复杂度：O(1)
    ...
    public int minCostClimbingStairs(int[] cost) {
        int minCostFirst = cost[0], minCostSecond = cost[1];
        int minCost = 0;
        for (int i = 2; i < cost.length; i++) {
            minCost = cost[i] + Math.min(minCostFirst, minCostSecond);
            minCostFirst = minCostSecond;
            minCostSecond = minCost;
        }
